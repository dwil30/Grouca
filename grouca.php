...
if(isset($_POST['Post'])) 
{
    ...
    {echo '<div style="color:red;">Please perform stock price lookup</div>';}
      ...
      $sql_insert =  $mysqli->query("INSERT INTO positions (Title, Status, Stock, Price, Sell, PriceSell, Sell2, PriceSell2, Buy, PriceBuy, Buy2, PriceBuy2, Gain, Loss, Margin, Notes, Date, Action, Trade, SetPrice) VALUES('" . $title . "','New', '" . $ticker . "', '" . $price. "','" . $sell . "','" . $price1 . "','" . $sell2 . "','" . $price2. "','" . $buy. "', '" . $price3. "','" . $buy2. "','" . $price4. "','" . $gain. "','" . $loss. "','" . $margin . "','" . $notes. "',CURRENT_TIMESTAMP,'" . $action. "','" . $trade. "','" . $setprice. "')") or die(mysqli_error());
...
if(isset($_POST['Open'])) 
{
    ...
    $_SESSION['ID'] = $ID;
    include("connect.php");  
    $sql_search = $mysqli->query("SELECT * FROM (SELECT * FROM positions Where TradeID = '".$ID."' ORDER BY DATE DESC) AS foo GROUP BY TradeID");
...
SET Title = '" . $title . "', Status = '" . $status . "', Stock = '".$ticker."', Price = '".$price."', Sell = '".$sell."', PriceSell = '".$price1."', Sell2 = '".$sell2."', PriceSell2 = '".$price2. "', Buy = '". $buy."', PriceBuy = '".$price3."', Buy2 = '".$buy2."', PriceBuy2 = '".$price4."', Gain = '" .$gain."', Loss = '".$loss."', Margin = '".$margin. "', Notes = '".$notes."', Date = CURRENT_TIMESTAMP, Action = '".$action."', Trade = '".$trade."', SetPrice = '".$setPrice."' WHERE ID = ".$_SESSION['results']['ID']."";
        $mysqli->query($sql_update);
        echo '<div style="color:red;">Trade Position Successfully Updated</div>';
    $_SESSION['Update'] = null;
    }   
    else {    
    $sql_update = $mysqli->query("INSERT INTO positions (Title, TradeID, Status, Stock, Price, Sell, PriceSell, Sell2, PriceSell2, Buy, PriceBuy, Buy2, PriceBuy2, Gain, Loss, Margin, Notes, Date, Action, Trade, SetPrice) VALUES('" . $title . "','" . $_SESSION['ID'] . "','" . $status . "', '" . $ticker . "', '" . $price. "','" . $sell . "','" . $price1 . "','" . $sell2 . "','" . $price2. "','" . $buy. "', '" . $price3. "','" . $buy2. "','" . $price4. "','" . $gain. "','" . $loss. "','" . $margin . "','" . $notes. "',CURRENT_TIMESTAMP,'" . $action. "','" . $trade. "','" . $setPrice. "')") or die(mysqli_error());
        if (($status == 'At Risk') or ($status == 'In Trouble')) {
            include 'send-update2.php';
            }
        echo '<div style="color:red;">Trade Position Successfully Updated</div>';
    $_SESSION['Update'] = null;
    }
...
<body>
<a href="users.php">Go to User Info</a>    
   <div class="w-row">
   ...
   <h3>New Daily Trade</h3>
    ...
    <table style="width:100%;table:1px solid black;tr:1px solid black;td:1px solid black;">
        ...
        <tr><th>Stock</th>
        <td><input  required style="width:20%" type="text" name="ticker" value="<?PHP echo htmlspecialchars($_POST['ticker']); ?>">&nbsp;&nbsp;<input name="Lookup" id="Lookup" type="submit" class="w-button button" value="Get Price"/>&nbsp;<b><?php echo isset($price) ? $price : '' ?></b></td></tr>
         <tr><th>Action</th>
        ...
        <br>
      <input name="Post" id="send" type="submit" class="w-button button" value="Post"/>
             <input name="Reset" id="send" type="reset" class="w-button button" value="Reset"/>
		</form>
    </div>
    ...
    <table style="width:100%;table:1px solid black;tr:1px solid black;td:1px solid black;">
      ...
      <?php 
            include("connect.php");  
            $sql_new = mysql_query("SELECT * FROM (SELECT * FROM positions ORDER BY DATE DESC) AS foo GROUP BY TradeID");
            while ($row = mysql_fetch_assoc($sql_new)) {
        ...
        <td>'.$row['SetPrice'].'</td> 
             ...
             <br><input name="Open" id="send" type="submit" class="w-button button" value="Update"/>
           </form>
        <br>
    ...
    <table style="width:100%;table:1px solid black;tr:1px solid black;td:1px solid black;">
     ...
     <?php 
            include("connect.php");  
            $sql_close = mysql_query("SELECT * FROM (SELECT * FROM positions ORDER BY DATE DESC) AS foo GROUP BY TradeID");
     ...
     <br><input name="Closed" id="send" type="submit" class="w-button button" value="Update"/>
            </form>
    ...
    <table style="width:100%;table:1px solid black;tr:1px solid black;td:1px solid black;">
        ...
        <tr><th>Stock</th>
        <td><input required style="width:20%" type="text" name="up_ticker" value="<?PHP if(empty($update['Stock'])){echo  htmlspecialchars($_POST['up_ticker']);} else {echo $update['Stock'];} ?>">&nbsp;<b><?PHP echo $update['Price']; ?>&nbsp;</b><input name="Lookup2" id="Lookup2" type="submit" class="w-button button" value="Get Price"/></td></tr>
          <tr><th>Trade</th>
        ...
        <tr><th>Notes</th>
            <td> <textarea id="message" name="up_notes" class="w-input msg-text"><?PHP if(empty($update['Notes'])){echo  htmlspecialchars($_POST['up_notes']);} else {echo $update['Notes'];} ?></textarea></td></tr>
             ...
             <br><input name="Change" type="submit" class="w-button button" value="Update"/>
     <input name="Reset" type="submit" class="w-button button" value="Cancel"/>
        </form>
