...
th, td, tr {
  ...
  border: 1px solid #ccc;
    ...
    <div class="w-container">
        ...
        include("base.php"); 
         $buy2 = mysql_query("select s.* from positions s join (select *, max(Timestamp) as maxdate from positions s group by TradeID) ss on s.TradeID = ss.TradeID and s.Timestamp = ss.maxdate where s.Status = 'New' and s.Buy2 != ''");
         $buy2_number = mysql_num_rows($buy2);
        $sell2 = mysql_query("select s.* from positions s join (select *, max(Timestamp) as maxdate from positions s group by TradeID) ss on s.TradeID = ss.TradeID and s.Timestamp = ss.maxdate where s.Status = 'New' and s.Sell2 != ''");
         $sell2_number = mysql_num_rows($sell2);
        $sql_query= mysql_query("select s.* from positions s join (select *, max(Timestamp) as maxdate from positions s group by TradeID) ss on s.TradeID = ss.TradeID and s.Timestamp = ss.maxdate where s.Status = 'New'");
...
$header = $count = 0;
  while ($row = mysql_fetch_assoc($open)) {
        ...
        echo '<td data-th="Send to IB"><button class="w-button button" style="margin-top:0px;">Send to IB</button>';            
        echo '</tr>';
                     }
  }
if ($count > 0 ) {
